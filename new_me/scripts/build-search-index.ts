...
function extractJSXText(source: string): string {
  ...
  let match: RegExpExecArray | null = jsxContentRegex.exec(content);
  while (match !== null) {
    ...
    if (
      ...
      text.length > 3 &&
      !text.match(
        /^(className|style|id|href|src|alt|target|rel|onClick|onMouseEnter|onMouseLeave|useEffect|useState|window\.|set\w+|const|let|var|function|return|if|for|while|\.map|\.filter|=>|=|{|}|\(|\)|\[|\])/
      ) &&
      ...
      !text.match(
        /\b(post|item|index|key|href|slug|result|entry|section|Link|Button|div|span|p|h1|h2|h3|h4|h5|h6)\.\w+\s*\?/
      ) &&
      !text.match(/\?\s*\(/) &&
      !text.match(/:\s*\(/) &&
    ...
    match = jsxContentRegex.exec(content);
  }
  ...
  while (match !== null) {
    ...
    if (
      !text.match(
        /^(https?:\/\/|\/|\.\/|className|id=|src=|alt=|href=|style=|onClick|useEffect|window\.|const|let|var|function|return)/i
      ) &&
      text.length > 30 &&
      /[a-zA-Z]{4,}/.test(text)
    ) {
  ...
  let extracted = textMatches
    ...
    .replace(
      /\b(text|font|mt|mb|px|py|bg|border|rounded|shadow|flex|grid|items|justify|gap|space|w|h|max|min|object|overflow|leading|tracking|uppercase|lowercase|capitalize|hidden|block|inline|sm|md|lg|xl|2xl|3xl|4xl|5xl|6xl|base|sm|xs|muted|foreground|background|accent|scroll)\S*/g,
      " "
    )
    // Remove common JSX patterns
    .replace(
      /\b(className|style|id|href|src|alt|target|rel|onClick|onMouseEnter|onMouseLeave|useEffect|useState|window|set\w+|const|let|var|function|return|if|for|while|map|filter|ul|li|div|span|p|h1|h2|h3|h4|h5|h6|a|img|svg|path|circle|rect|g|defs|clipPath|linearGradient|stop)\b/gi,
      " "
    )
    ...
    .replace(
      /\b(post|item|index|key|href|slug|result|entry|section|Link|Button|div|span|p|h1|h2|h3|h4|h5|h6)\s*\.\s*\w+/gi,
      " "
    ...
    .replace(
      /\b(post\.slug|post\.|item\.|index\.|key\s*=|href\s*=|slug\s*=|result\.|entry\.|section\.|Link\s*\(|Button\s*\(|div\s*\(|span\s*\(|p\s*\(|h1\s*\(|h2\s*\(|h3\s*\(|h4\s*\(|h5\s*\(|h6\s*\()/gi,
      " "
    )
    // Remove fragments that are clearly code
    .replace(
      /\b(post|item|index|key|href|slug|result|entry|section|Link|Button|div|span|p|h1|h2|h3|h4|h5|h6|map|filter|forEach|reduce|find|some|every|includes|split|join|replace|trim|toLowerCase|toUpperCase|push|pop|shift|unshift|slice|splice|concat|sort|reverse|indexOf|lastIndexOf|substring|substr|charAt|charCodeAt|fromCharCode|match|search|replace|exec|test|toString|valueOf|hasOwnProperty|isPrototypeOf|propertyIsEnumerable|toLocaleString|toJSON|parse|stringify|keys|values|entries|assign|create|defineProperty|defineProperties|getOwnPropertyDescriptor|getOwnPropertyNames|getOwnPropertySymbols|getPrototypeOf|setPrototypeOf|is|seal|freeze|preventExtensions|isExtensible|isSealed|isFrozen|hasOwn|get|set|delete|has|clear|size|forEach|keys|values|entries|Symbol|iterator|toStringTag|hasInstance|isConcatSpreadable|match|replace|search|split|species|toPrimitive|unscopables)\b(?=\s*[=:(.?])/gi,
      " "
    ) // Only if followed by code operators
    ...
    .replace(
      /\b(post|item|index|key|href|slug|result|entry|section|Link|Button|div|span|p|h1|h2|h3|h4|h5|h6)\s+[=:(.?]/gi,
      " "
    )
    ...
    .replace(
      /,\s*;\s*\b(Link|key|href|post|slug|index|item|result|entry|section|Button|div|span|p|h1|h2|h3|h4|h5|h6)\b/gi,
      " "
    )
    .replace(/,\s*;\s*\w+/g, " ") // Remove ", ; word" patterns
    .replace(
      /\b(Link|key|href|post|slug|index|item|result|entry|section|Button|div|span|p|h1|h2|h3|h4|h5|h6)\b(?=\s*$)/gi,
      " "
    ) // Remove standalone code words at end
    .replace(
      /\b(Link|key|href|post|slug|index|item|result|entry|section|Button|div|span|p|h1|h2|h3|h4|h5|h6)\b(?=\s*[,;])/gi,
      " "
    ) // Remove code words before punctuation
    ...
    .replace(
      /\b(showRealImage|setShowRealImage|updateHeight|setWindowHeight|window|addEventListener|removeEventListener|useEffect|useState)\b/gi,
      " "
    ) // Remove React/JS patterns
    .replace(/;\s*\(/g, " ") // Remove "; (" patterns
    .replace(/\)\s*;/g, " ") // Remove ");" patterns
    .replace(/\(\s*\(/g, " ") // Remove "((" patterns
    .replace(/\)\s*\)/g, " ") // Remove "))" patterns
...
function extractPapersFromNewspaper(): SearchEntry[] {
  ...
  const paperRegex =
    /\{\s*date:\s*"([^"]*)",\s*dateSort:\s*"([^"]*)",\s*headline:\s*"([^"]*)",\s*content:\s*"([^"]*)",\s*authors:\s*"([^"]*)",\s*paperTitle:\s*"([^"]*)",\s*venue:\s*"([^"]*)",/g;

  let match: RegExpExecArray | null = paperRegex.exec(arrayContent);
  while (match !== null) {
    ...
    match = paperRegex.exec(arrayContent);
  }
  ...
  const flexiblePaperRegex =
    /\{\s*date:\s*"([^"]*)",[\s\S]*?headline:\s*"([^"]*)",[\s\S]*?content:\s*"([^"]*)",[\s\S]*?authors:\s*"([^"]*)",[\s\S]*?paperTitle:\s*"([^"]*)",[\s\S]*?venue:\s*"([^"]*)",/g;

  // Reset and try flexible regex if we didn't find papers
  if (entries.length === 0) {
    let flexMatch: RegExpExecArray | null = flexiblePaperRegex.exec(arrayContent);
    while (flexMatch !== null) {
      ...
      flexMatch = flexiblePaperRegex.exec(arrayContent);
    }
  }
