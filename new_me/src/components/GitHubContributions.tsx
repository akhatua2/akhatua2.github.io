...
export default function GitHubContributions({
  ...
  weeks.forEach((week, weekIndex) => {
    ...
    if (firstDayWithDate && firstDayWithDate.date) {
    ...
    <div className="p-6">
      {showStats && (
        <div className="mb-4 flex items-center justify-between">
          <div>
            {loading ? (
              <div className="h-4 w-48 bg-gray-100 dark:bg-gray-900 rounded animate-pulse"></div>
            ) : (
                  ...
                  {week.map((day, dayIndex) => {
                    ...
                    return (
                      <div
                        ...
                        className={`h-[10px] w-full rounded-[2px] ${getColor(day.level, loading)} ${isEmpty ? '' : 'hover:ring-1 hover:ring-accent cursor-pointer'}`}
                        style={{ 
                          aspectRatio: '1',
                          opacity: loading ? 1 : (isAnimating ? 1 : (dataLoaded ? 0 : 1)), // Start at 0, animate to 1
                          transition: isAnimating ? `opacity 200ms ease-out ${delay}ms, background-color 300ms ease-out ${delay}ms` : 'none',
                        }}
                        title={isEmpty || loading ? '' : `${day.count} contributions on ${new Date(day.date).toLocaleDateString()}`}
                        onClick={() => {
                          if (!isEmpty && !loading) {
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-muted">
        ...
        <div className="flex gap-[2px]">
          {[0, 1, 2, 3, 4].map((level) => (
            <div
              key={level}
              className={`h-[10px] w-[10px] rounded-[2px] ${getColor(level)}`}
            />
          ))}
        </div>
        <span>More</span>
      ...
      {selectedDay && (
        <div className="mt-4 p-3 bg-muted/50 rounded-lg text-sm">
          <p className="font-semibold text-foreground">
            ...
            on {new Date(selectedDay.date).toLocaleDateString("en-US", {
              weekday: "long",
              year: "numeric",
              month: "long",
              day: "numeric",
